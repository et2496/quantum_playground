...
def render_qaoa_intro():
    ...
    st.write("""
        ### Basics
        The QAOA is an algorithm to solve binary decisions. For example, in logistics, a routing problem is a set of binary decisions such as "Where do I go next?"
        The solution space grows exponentially with the number of locations.
        ...
        All in all with classical methods, we quickly get to a point where we can't get a global minimum for the exact system for a given time.
        ...
        It is a common problem in a variety of processes and industries. Everyday, companies have to make one of the above compromises.
        What could we change when we switch from a classical solution to a hybrid quantum solution?
        We describe the system with a problem Hamiltonian and a mixer Hamiltonian. We then use a classical optimizer to improve the solution similar to our VQE optimizer.
        ...
        Before we look at an example, it is important to mention that trapped ion architectures could potentially benefit from their all-to-all qubit connection that decreases the circuit depths and ultimately decreases the accumulated error.

        ### Example
        A logistics company wants to split their route into two operating zones. This is called a MaxCut problem. We try to find the best split for the network.
        In this example, we can directly see why we need other algorithms besides the brute force algorithm. 
        The brute force algorithm grows exponatially with the number of nodes and, therefore, is not applicable for larger systems.
        ...
        The hope for quantum solutions is that quantum can represent a superposition of many solutions at once and, therefore, solve it quicker than classical computers.
        The example should showcase that the QAOA algorithm works. The following parameters can be adjusted to squeeze the solution:
